 ...
 * Declare class
 */
class YAYDP_Cart_Discount_Encouragement extends  \YAYDP\Abstracts\YAYDP_Encouragement {
	...
	public function get_content() {
		...
		if ( strpos( $replaced_content, '[discount_value]' ) !== false || strpos( $replaced_content, '[action]' ) !== false ) {
			return '';
		}
	...
	public function get_raw_content() {
		$encouraged_settings = \YAYDP\Settings\YAYDP_Cart_Discount_Settings::get_instance()->get_encouraged_notice_settings();
		...
		foreach ( $this->conditions_encouragements as $condition_encouragement ) {
			...
			switch ( $condition_encouragement['type'] ) {
				case YAYDP_Cart_Subtotal_Price_Condition::INCOMPLETE_KEY:
					...
					$action = apply_filters( 'yaydp_cart_discount_encouraged_notice_action', $action, $this->get_raw_content(), $condition_encouragement['type'], $missing_value );
					break;
				...
				case YAYDP_Logged_Customer_Condition::INCOMPLETE_KEY:
					...
					$action = apply_filters( 'yaydp_cart_discount_encouraged_notice_action', $action, $this->get_raw_content(), $condition_encouragement['type'], null );
					break;
