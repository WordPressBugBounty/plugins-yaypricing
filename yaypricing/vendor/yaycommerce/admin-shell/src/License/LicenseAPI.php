...
class LicenseAPI {
    ...
    public static function activate_license( string $store_url, int $item_id, string $license_key ): array {
        try {
            $url      = $store_url . '?edd_action=activate_license&item_id=' . $item_id
                . '&license=' . rawurlencode( $license_key ) . '&url=' . home_url();
            ...
            if ( isset( $response->success ) && $response->success ) {
                return [
                    'success'       => true,
                    'license'       => $response->license ?? 'valid',
    ...
    public static function deactivate_license( string $store_url, int $item_id, string $license_key ): array {
        try {
            $url = $store_url . '?edd_action=deactivate_license&item_id=' . $item_id
                . '&license=' . rawurlencode( $license_key ) . '&url=' . home_url();
            $raw = wp_remote_get( $url );

            if ( is_wp_error( $raw ) ) {
                return [ 'success' => false, 'is_server_error' => true ];
            }
            ...
            return [
                ...
                'license' => $response->license ?? 'deactivated',
            ];
        } catch ( \Error $error ) {
            return [ 'success' => false, 'is_server_error' => true ];
        }
    ...
    public static function check_license( string $store_url, int $item_id, $license_key ): array {
        try {
            $url      = $store_url . '?edd_action=check_license&item_id=' . $item_id
                . '&license=' . rawurlencode( (string) $license_key ) . '&url=' . home_url();
            $raw      = wp_remote_get( $url );
    ...
    public static function get_error_message( string $message ): string {
        $messages = [
            ...
            'server_error'          => 'Your license could not be activated because of server error.',
        ];
