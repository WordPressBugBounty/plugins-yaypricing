...
class License {
    ...
    public function activate( string $license_key ): array {
        $activate_response = LicenseAPI::activate_license(
            $this->adapter->get_store_url(),
            $this->adapter->get_item_id(),
            $license_key
        );
    ...
    public function update(): array {
        ...
        $response    = LicenseAPI::check_license(
            $this->adapter->get_store_url(),
            $this->adapter->get_item_id(),
            $license_key
        );
    ...
    public function get_renewal_url(): string {
        return $this->adapter->get_store_url()
            . 'checkout/?edd_license_key=' . $this->license_key
            . '&download_id=' . $this->adapter->get_item_id();
    }

    public function get_upgrade_url(): string {
        $payment_id = $this->license_info['payment_id'] ?? '';
        return $this->adapter->get_store_url()
            . 'checkout/purchase-history/?view=upgrades&action=manage_licenses&payment_id=' . $payment_id;
    }
}
