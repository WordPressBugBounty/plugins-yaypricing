...
use YayCommerce\AdminShell\Support\Slug;
...
class LicenseHandler {
    ...
    public function do_hooks(): void {
        ...
        if ( ! $this->license->is_active() || $this->license->is_expired() ) {
            add_filter(
                'plugin_action_links_' . $this->adapter->get_plugin_basename(),
    ...
    public function enqueue_license_scripts(): void {
        if ( ! isset( $_GET['page'] ) || 'yaycommerce-licenses' !== $_GET['page'] ) { // phpcs:ignore WordPress.Security.NonceVerification
            return;
        }
        ...
        wp_enqueue_script(
            'yaycommerce-license',
            $assets_url . 'js/license.js',
            [ 'jquery' ],
            $this->adapter->get_plugin_version(),
            true
        );
        wp_localize_script(
            'yaycommerce-license',
            Slug::to_var_name( $slug ) . 'LicenseData',
            [
                'slug'        => $slug,
                'apiSettings' => [
                    'restNonce' => wp_create_nonce( 'wp_rest' ),
                    'restUrl'   => esc_url_raw( rest_url( Slug::to_var_name( $slug ) . '/v1' ) ),
                    'adminUrl'  => admin_url(),
                ],
            ]
        );
    ...
    public function register_licensing_plugins( array $plugins = [] ): array {
        $plugin_data = [
            'name'     => $this->adapter->get_plugin_name(),
            'slug'     => $this->adapter->get_plugin_slug(),
            'basename' => $this->adapter->get_plugin_basename(),
            'file'     => $this->adapter->get_plugin_file(),
            'url'      => $this->adapter->get_store_link(),
            'item_id'  => $this->adapter->get_item_id(),
        ];
    ...
    public function not_activate_license_notice(): void {
        ...
        <div class="error">
            ...
            printf(
                esc_html__( '%1$s license key is required. %2$sPlease enter your license key to start using the plugin%3$s.', 'yaycommerce' ),
                esc_html( $name ),
                '<a href="' . esc_url( admin_url( 'admin.php?page=yaycommerce-licenses' ) ) . '">',
                '</a>'
            );
            ?>
            </p>
    ...
    public function plugin_notifications( string $file ): void {
        ...
        if ( ! $this->license->is_active() || $this->license->is_expired() ) {
            ...
            <?php if ( ! $this->license->is_active() ) : ?>
                <div class="update-message notice inline notice-warning notice-alt">
                    <p>
                        <a href="<?php echo esc_url( admin_url( 'admin.php?page=yaycommerce-licenses' ) ); ?>">
                            <?php esc_html_e( 'Please activate your license for access to premium features and automatic updates', 'yaycommerce' ); ?>
                        </a>.
                    </p>
                </div>
            <?php endif; ?>
            <?php if ( $this->license->is_expired() ) : ?>
                <div class="update-message notice inline notice-warning notice-alt">
                    <p class="license_expired_text">
                        ...
                        <a target="_blank" href="<?php echo esc_url( $this->license->get_renewal_url() ); ?>"><?php echo esc_html( 'renew this license' ); ?></a>
    ...
    public function add_license_action_link( array $action_links ): array {
        $link = [
            'license' => '<a href="' . admin_url( 'admin.php?page=yaycommerce-licenses' ) . '">'
                . esc_html__( 'Enter license key', 'yaycommerce' ) . '</a>',
        ];
    ...
    public function auto_update(): void {
        ...
        $args = [
            'version' => $this->adapter->get_plugin_version(),
            'license' => $license_key,
            'author'  => 'YayCommerce',
            'item_id' => $this->adapter->get_item_id(),
        ];
        ...
        new EDD_SL_Plugin_Updater(
            $this->adapter->get_store_url(),
            $this->adapter->get_plugin_file(),
            $args
        );
    }
