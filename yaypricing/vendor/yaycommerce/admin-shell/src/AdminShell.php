...
namespace YayCommerce\AdminShell;

use YayCommerce\AdminShell\Contracts\AddonHostAdapter;
use YayCommerce\AdminShell\Contracts\PluginMenuAdapter;
use YayCommerce\AdminShell\License\Contracts\LicenseConfigAdapter;
use YayCommerce\AdminShell\License\LicenseHandler;
use YayCommerce\AdminShell\License\PluginInfoFactory;
use YayCommerce\AdminShell\Menu\ExternalPluginMenuAdapter;
use YayCommerce\AdminShell\Menu\PluginSubmenu;
use YayCommerce\AdminShell\Menu\MenuSuppressor;
use YayCommerce\AdminShell\Menu\PagesRouter;
use YayCommerce\AdminShell\Menu\SubmenuPositioner;
use YayCommerce\AdminShell\Menu\TopLevelMenu;
use YayCommerce\AdminShell\Pages\RecommendedPluginsPage;
use YayCommerce\AdminShell\Registry\AddonBridge;
...
use YayCommerce\AdminShell\Support\Constants;
...
class AdminShell {
    ...
    public static function boot(): void {
        ...
        $ns           = __NAMESPACE__; // YayCommerce\AdminShell or ScopedPrefix\YayCommerce\AdminShell
        $parts        = explode( '\\', $ns );
        ...
        $GLOBALS['yaycommerce_admin_shell_versions'][ self::$prefix ] = [
            'version'  => self::VERSION,
            'registry' => $instance->registry,
            'boot_cb'  => [ static::class, 'do_shell_registration' ],
        ];
    ...
    private static function register_plugin_links( PluginMenuAdapter $adapter ): void {
        ...
        add_filter( 'plugin_action_links_' . $basename, function ( array $links ) use ( $adapter ) {
            ...
            if ( ! empty( $menu_slug ) ) {
                ...
                $new['settings'] = '<a href="' . esc_url( $url ) . '">'
                    . esc_html( $adapter->get_settings_label() ) . '</a>';
            }

            $pro_url = $adapter->get_pro_url();
            if ( ! empty( $pro_url ) ) {
                $new['go-pro'] = '<a href="' . esc_url( $pro_url ) . '" target="_blank" style="color:#00a32a;font-weight:700;">'
                    . esc_html__( 'Go Pro', 'yaycommerce' ) . '</a>';
            }
        ...
        add_filter( 'plugin_row_meta', function ( array $meta, string $file ) use ( $adapter, $basename ) {
            ...
            if ( ! empty( $docs_url ) ) {
                $meta[] = '<a href="' . esc_url( $docs_url ) . '" target="_blank">'
                    . esc_html__( 'Docs', 'yaycommerce' ) . '</a>';
            }
            $meta[] = '<a href="https://yaycommerce.com/support" target="_blank">'
                . esc_html__( 'Support', 'yaycommerce' ) . '</a>';
            return $meta;
        }, 10, 2 );
    }
